...
plt.xlim(left=8, right=14)
plt.title(device_name + ' 4k write QD=1 latency histogram')

sns_plot.fig.set_figwidth(6)
sns_plot.fig.set_figheight(3)
sns_plot.set(xlabel='latency, µs')

sns_plot.savefig(device_name + 'cd6_clat_write.svg', format='svg', transparent=True)
sns_plot.savefig(device_name + 'cd6_clat_write.png', format='png')